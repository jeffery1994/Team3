...
Hull HeightMapCollisionShape::cubeHull = Hull();

HeightMapCollisionShape::HeightMapCollisionShape(const int w, const int h, const float s)
{
	halfDims = Vector3(w*s,0.1f,h*s);

	if (cubeHull.GetNumVertices() == 0)
	{
		ConstructCubeHull(w,h);
	}
}
...
Matrix3 HeightMapCollisionShape::BuildInverseInertia(float invMass) const
{
	Matrix3 inertia;
...
void HeightMapCollisionShape::GetCollisionAxes(
...
Vector3 HeightMapCollisionShape::GetClosestPoint(const Vector3& point) const
{
...
void HeightMapCollisionShape::GetMinMaxVertexOnAxis(
	const Vector3& axis,
	Vector3& out_min,
...
void HeightMapCollisionShape::GetIncidentReferencePolygon(
	const Vector3& axis,
...
void HeightMapCollisionShape::DebugDraw() const
{
...
void HeightMapCollisionShape::ConstructCubeHull(const int w, const int h)
{
	//Vertices
	int c = 0;
	for (int i = - (w/2); i < (w/2); ++i) {
		for (int j = -(h/2); j < (h/2); ++j) {
			...
			cubeHull.AddVertex(Vector3(i , 1, j));
			cubeHull.AddVertex(Vector3(i, 1, (j + 1)));
			cubeHull.AddVertex(Vector3((i + 1), 1, (j + 1)));
			cubeHull.AddVertex(Vector3((i + 1), 1, j));
			...
			int face1[] = { c + 0, c + 1, c + 2, c + 3 };
			int face2[] = { c + 7, c + 6, c + 5, c + 4 };
			int face3[] = { c + 5, c + 6, c + 2, c + 1 };
			int face4[] = { c + 0, c + 3, c + 7, c + 4 };
			int face5[] = { c + 6, c + 7, c + 3, c + 2 };
			int face6[] = { c + 4, c + 5, c + 1, c + 0 };
...
float HeightMapCollisionShape::GetRayIntersection(Vector3 origin, Vector3 direction) {
	float d = -1.0f;
...
Vector3 HeightMapCollisionShape::GetPlaneIntersection(Vector3 pCenter, Vector3 pNormal, Vector3 lCenter, Vector3 lDirection) {
